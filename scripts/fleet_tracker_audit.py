#!/usr/bin/env python3
"""One-shot audit for fleet repo todo.sr.ht tracker label conformance."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--labels-json", type=pathlib.Path, help="fixture JSON from `srht todo labels list --json --all`; avoids network")
    parser.add_argument("--srht-bin", default="srht")
    args = parser.parse_args(argv)

    root = pathlib.Path(__file__).resolve().parent.parent
    config = tomllib.loads((root / "fleet.toml").read_text())
    labels = load_label_fixture(args.labels_json) if args.labels_json else list_tracker_labels(srht_bin=args.srht_bin)
    worst = 0
